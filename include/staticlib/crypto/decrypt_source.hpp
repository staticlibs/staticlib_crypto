...
public:
    ...
    std::streamsize read(sl::io::span<char> span) {
        ...
        if (0 == read_from_src) {
            ...
            auto written_flushed = BIO_write(bsrc.get(), std::addressof(eof), 1);
            if (written_flushed <= 0) throw crypto_exception(TRACEMSG(
                    "'BIO_write' eof error, to_write: [" + sl::support::to_string(read_from_src) + "]," +
                    " written: [" + sl::support::to_string(written_flushed) + "]," +
                    " code: [" + sl::support::to_string(ERR_get_error()) + "]"));
            // read and return
